...
public class EpicGames {
	...
	public static void main(String[] args) {
		File file = new File("E:\\BreachDatabases\\Epic_Games\\EpicGames_RF\\Data\\EpicGames.com_August_2016_vB_.txt");
		System.out.println("Currently converting file: EpicGames.com_August_2016_vB_.txt");
		readFile(file.getAbsolutePath());			
	}
	
	private static String createMetaJSON() {
		String metajson = "{\"index\" : {\"_index\": \"breach-epicgames\", \"_type\" : \"_doc\", \"_id\": \"" + lineCount + "\"}}";
		//System.out.println(metajson);
		return metajson;
	}
	
	private static String writeFile(String metajson, String datajson) {
		File f = new File("E:\\BreachDatabases\\Epic_Games\\EpicGames_RF\\Data\\output\\epicgames_"+outputFileNumber.toString()+".json");
		if(!f.exists()){
			  try {
		  ...
		  try {
			  ...
			  if (arLines.length >= 3) {
				String index = "{\"Username\": \"" + escape(arLines[0]).replaceAll("\\s+","").replace("\0", "") + "\"," +
								...
								"\"Password\": \"" + escape(arLines[2]).replaceAll("\\s+","").replace("\0", "") + "\"}";
